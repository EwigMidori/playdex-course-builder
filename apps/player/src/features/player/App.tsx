    ...
    Promise.all(
      course.chapters.flatMap((chapter) => {
        ...
        return steps.map(async (step) => {
          const bank = await CourseAssetClient.loadQuestionBank(chapter, step.sequence_id ?? "step1");
...
function useStoryBundle(course: CourseRecord | null, chapter: ChapterRecord | null, stepId: string | null) {
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    setState(emptyState());
    Promise.all([CourseAssetClient.loadStep(chapter, stepId), CourseAssetClient.loadQuestionBank(chapter, stepId)]).then(([step, bank]) => {
      if (!alive) {
        return;
      }
