...
import { ViewerAssetClient, ViewerAssetPaths } from "../../data";
...
function useCourseCatalogState() {
  ...
  React.useEffect(() => {
    let alive = true;
    ViewerAssetClient.loadCourseCatalog().then((result) => {
      if (alive) {
...
function useTextbook(lesson: LessonOption | null) {
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    setState(emptyState());
    ViewerAssetClient.fetchText(ViewerAssetPaths.textbook(lesson)).then((result) => {
      if (alive) {
        setState({ loading: false, data: result.data, error: result.error, path: result.path });
      }
    });
...
function useStoryData(lesson: LessonOption | null, stepId: string) {
  ...
  React.useEffect(() => {
    ...
    ViewerAssetClient.loadManifest(lesson).then((result) => {
      if (alive) {
  ...
  React.useEffect(() => {
    ...
    setStep(emptyState());
    setBank(emptyState());
    Promise.all([ViewerAssetClient.loadStep(lesson, stepId), ViewerAssetClient.loadQuestionBank(lesson, stepId)]).then(([stepResult, bankResult]) => {
      if (alive) {
...
function useRelevance(lesson: LessonOption | null) {
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    setState(emptyState());
    ViewerAssetClient.loadRelevance(lesson).then((result) => {
      if (alive) {
        setState({ loading: false, data: result.data, error: result.error, path: result.path });
      }
    });
...
function useQuestionIndex(lesson: LessonOption | null) {
  ...
  React.useEffect(() => {
    ...
    ViewerAssetClient.loadManifest(lesson).then(async (manifest) => {
      ...
      const banks = await Promise.all(
        steps.map((step) => ViewerAssetClient.loadQuestionBank(lesson, step.sequence_id ?? lesson.defaultStep ?? "step1"))
      );
    ...
    <section className="grid gap-4 xl:grid-cols-3">
      ...
      <Panel title="Primary Files" subtitle="read-only pipeline sources">
        <div className="space-y-2 text-xs">
          <FileLine path={ViewerAssetPaths.textbook(currentLesson)} />
          <FileLine path={ViewerAssetPaths.manifest(currentLesson)} />
          <FileLine path={`${currentLesson.paths.guidedStoryDir}/<step>/question_bank.json`} />
          <FileLine path={ViewerAssetPaths.relevance(currentLesson)} />
          <FileLine path={ViewerAssetPaths.plainArtifactsDir(currentLesson)} />
        </div>
      </Panel>
