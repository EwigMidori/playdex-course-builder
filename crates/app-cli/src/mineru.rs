...
pub struct MineruConverter;

impl MineruConverter {
    pub fn convert_lesson(
        ...
        let result = Self::run_convert(&config, &token, &raw_pdf_path, &stage, lesson.lesson_id());
        match result {
            Ok(()) => Ok(()),
    ...
    pub fn convert_exam_pdfs(repo: &RepoPaths, resume: bool) -> Result<usize, ConvertError> {
        let raw_pdf_paths = Self::list_exam_pdfs(repo)?;
        ...
        for raw_pdf_path in raw_pdf_paths {
            let stem = Self::pdf_stem(&raw_pdf_path)?;
            ...
            let result = Self::run_convert(&config, &token, &raw_pdf_path, &stage, &data_id);
            match result {
                Ok(()) => converted += 1,
    ...
    fn run_convert(
        ...
        let upload = Self::request_upload(config, token, raw_pdf_path, data_id)?;
        Self::upload_file(config, &upload.upload_url, raw_pdf_path)?;
        let completed = Self::poll_until_done(config, token, &upload.batch_id)?;
        ...
        Self::extract_archive(&stage.archive_path, &stage.artifacts_dir)?;
        let plain_text_bytes =
            Self::materialize_plain_text(&stage.artifacts_dir, &stage.plain_text_path)?;
...
impl PreparedOutput {
    fn prepare(final_output_dir: PathBuf, staging_prefix: &str) -> Result<Self, ConvertError> {
        ...
        let staging_dir = MineruConverter::create_staging_dir(&parent, staging_prefix)?;
        Ok(Self {
            final_output_dir,
...
impl MineruConverter {
    fn request_upload(
        ...
        let body: Value = response
            .json()
            .map_err(|source| ConvertError::CreateUploadRequest { source })?;
        let data = Self::api_data(body, ApiOperation::CreateUpload)?;
        let batch_id = Self::required_string(&data, "batch_id")
            .map_err(|field| ConvertError::CreateUploadField { field })?;
        let upload_url = Self::first_upload_url(&data).ok_or(ConvertError::CreateUploadField {
    ...
    fn poll_until_done(
        ...
        loop {
            ...
            if let Some(result) = extract_results.first() {
                let state = Self::required_string(result, "state").map_err(|field| {
                ...
                if state == "done" {
                    let full_zip_url =
                        Self::required_string(result, "full_zip_url").map_err(|field| {
                            ConvertError::PollField {
                                batch_id: batch_id_string.clone(),
                                field,
                            }
    ...
    fn download_archive(
        ...
        let mut output =
            File::create(archive_path).map_err(|source| ConvertError::DownloadWrite {
    ...
    fn extract_archive(archive_path: &Path, artifacts_dir: &Path) -> Result<(), ConvertError> {
        ...
        let archive_file =
            File::open(archive_path).map_err(|source| ConvertError::ZipEntryWrite {
                path: archive_path.to_path_buf(),
                source,
            })?;
        let mut archive =
            ZipArchive::new(archive_file).map_err(|source| ConvertError::ZipOpen {
                path: archive_path.to_path_buf(),
                source,
            })?;
        ...
        for index in 0..archive.len() {
            ...
            let relative_path = Self::validate_entry_path(&entry_name).map_err(|reason| {
                ConvertError::ZipEntryInvalid {
                    archive_path: archive_path.to_path_buf(),
                    entry: entry_name.clone(),
                    reason,
                }
            })?;
            let output_path = Self::contained_artifact_path(
                archive_path,
                artifacts_dir,
                &entry_name,
                &relative_path,
    ...
    fn materialize_plain_text(
        ...
        let markdowns = Self::list_markdown_files(artifacts_dir)?;
...
impl MineruConverter {
    ...
    fn first_upload_url(data: &Value) -> Option<String> {
        Self::first_string_at(data.get("file_urls"))
            .or_else(|| Self::first_string_at(data.get("files")))
    }
    ...
    fn validate_entry_path(entry_name: &str) -> Result<PathBuf, &'static str> {
        ...
        if Self::has_windows_drive_prefix(entry_name) {
    ...
    fn contained_artifact_path(
        ...
        let output_path = Self::normalize_lexical_path(&artifacts_dir.join(relative_path));
    ...
    fn list_markdown_files(artifacts_dir: &Path) -> Result<Vec<PathBuf>, ConvertError> {
        ...
        Self::collect_markdown_files(artifacts_dir, &mut markdowns)?;
        markdowns.sort();
        Ok(markdowns)
    }
    ...
    fn collect_markdown_files(
        ...
        for entry in entries {
            ...
            if file_type.is_dir() {
                Self::collect_markdown_files(&path, markdowns)?;
