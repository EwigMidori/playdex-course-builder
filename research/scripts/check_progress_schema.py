...
def validate_phases(
    ...
    for phase_index, phase in enumerate(value):
        ...
        expect_exact_keys(label, phase, PHASE_KEYS, errors)
        ...
        expect_type(f"{label}.description", phase.get("description"), str, errors)
        expect_type(f"{label}.status", phase.get("status"), str, errors)
        ...
        for task_index, task in enumerate(tasks):
            ...
            expect_exact_keys(task_label, task, TASK_KEYS, errors)
            task_id = task.get("id")
            ...
            expect_type(f"{task_label}.description", task.get("description"), str, errors)
            ...
            validate_string_list(f"{task_label}.crate_scope", task.get("crate_scope"), errors)
            validate_string_list(f"{task_label}.depends_on", task.get("depends_on"), errors)
            expect_type(f"{task_label}.status", task.get("status"), str, errors)
