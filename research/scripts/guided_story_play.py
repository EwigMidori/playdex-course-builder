...
class GuidedStoryPlayCli:
    def main() -> int:
        parser = GuidedStoryPlayCli.build_parser()
        args = parser.parse_args()

        try:
            step_path = GuidedStoryPlayCli.resolve_step_path(args)
            step = GuidedStoryPlayCli.load_json(step_path)
            GuidedStoryPlayCli.validate_step(step_path, step)
            return GuidedStoryPlayCli.play_step(step_path, step, args)
    ...
    def resolve_step_path(args: argparse.Namespace) -> Path:
        ...
        manifest = GuidedStoryPlayCli.load_json(manifest_path)
    ...
    def render_line(line: str, term_catalog: dict[str, Any], plain: bool) -> str:
        def replacer(match: re.Match[str]) -> str:
            ...
            return GuidedStoryPlayCli.style(display, bold=True, plain=plain)
    ...
    def print_step_header(step_path: Path, step: dict[str, Any], plain: bool, show_source: bool) -> None:
        print(GuidedStoryPlayCli.style(f"{step.get('lesson_id', '?')} / {step.get('sequence_id', '?')}", bold=True, plain=plain))
        print(GuidedStoryPlayCli.style(step_path.as_posix(), dim=True, plain=plain))
        if show_source:
            source = step.get("source", {})
            print(GuidedStoryPlayCli.style(f"source.plain_text: {source.get('plain_text', '')}", dim=True, plain=plain))
            related = source.get("related") or []
            for rel in related:
                print(GuidedStoryPlayCli.style(f"source.related: {rel}", dim=True, plain=plain))
        print("")
    ...
    def play_step(step_path: Path, step: dict[str, Any], args: argparse.Namespace) -> int:
        ...
        GuidedStoryPlayCli.print_step_header(step_path, step, plain, args.show_source)

        while 0 <= index < len(screens):
            screen = screens[index]
            GuidedStoryPlayCli.print_screen(screen, term_catalog, index, len(screens), plain, args.show_focus_terms)

            if screen.get("type") == "exercise" and not args.skip_exercises:
                result = GuidedStoryPlayCli.run_exercise(screen, plain)
                if result:
            ...
            action = GuidedStoryPlayCli.prompt_nav(term_catalog, plain)
            if action == "next":
                index += 1
            elif action == "back":
                index = max(0, index - 1)
                ...
                GuidedStoryPlayCli.show_term(term_id, term_catalog, plain)

        print("")
        print(GuidedStoryPlayCli.style("Playback finished.", bold=True, plain=plain))
        if results:
            GuidedStoryPlayCli.summarize_results(results, plain)
        return 0
    ...
    def print_screen(
        ...
        print(GuidedStoryPlayCli.style(header, dim=True, plain=plain))
        for line in screen.get("lines", []):
            print(GuidedStoryPlayCli.render_line(line, term_catalog, plain))
        ...
        if isinstance(formula, dict):
            ...
            if latex:
                print(GuidedStoryPlayCli.style(f"formula: {latex}", dim=True, plain=plain))
            if spoken:
                print(GuidedStoryPlayCli.style(f"spoken: {spoken}", dim=True, plain=plain))

        if show_focus_terms:
            focus_terms = screen.get("focus_terms") or []
            if focus_terms:
                print(GuidedStoryPlayCli.style("focus terms:", dim=True, plain=plain))
                for term_id in focus_terms:
                    GuidedStoryPlayCli.show_term(term_id, term_catalog, plain, indent="  ", prefix="- ")
        print("")
    ...
    def run_exercise(screen: dict[str, Any], plain: bool) -> ExerciseResult | None:
        ...
        print(GuidedStoryPlayCli.style(prompt, bold=True, plain=plain))
        ...
        if kind == "single_choice":
            ...
            picked = GuidedStoryPlayCli.parse_single_choice_answer(raw, len(options))
        ...
        if kind == "fill_in_blank":
            ...
            is_correct = GuidedStoryPlayCli.normalize_text(raw) in answers if answers else None
            GuidedStoryPlayCli.print_exercise_feedback(exercise, is_correct, plain)
            return ExerciseResult(prompt=prompt, user_answer=raw or None, is_correct=is_correct)
        ...
        if kind == "short_reflection":
            ...
            if explanation:
                print(GuidedStoryPlayCli.style(f"note: {explanation}", dim=True, plain=plain))
            return ExerciseResult(prompt=prompt, user_answer=raw or None, is_correct=None)

        print(GuidedStoryPlayCli.style(f"unsupported exercise kind: {kind}", dim=True, plain=plain))
        return None
    ...
    def print_exercise_feedback(exercise: dict[str, Any], is_correct: bool | None, plain: bool) -> None:
        if is_correct is True:
            print(GuidedStoryPlayCli.style("correct", bold=True, plain=plain))
        elif is_correct is False:
            print(GuidedStoryPlayCli.style("incorrect", bold=True, plain=plain))
        explanation = exercise.get("explanation")
        if explanation:
            print(GuidedStoryPlayCli.style(f"why: {explanation}", dim=True, plain=plain))
        print("")
    ...
    def prompt_nav(term_catalog: dict[str, Any], plain: bool) -> str:
        while True:
            ...
            if raw == "q":
                return "quit"
            if raw == "l":
                print(GuidedStoryPlayCli.style("available terms:", dim=True, plain=plain))
    ...
    def show_term(
        ...
        *,
        ...
        print(f"{indent}{prefix}{GuidedStoryPlayCli.style(line, bold=True, plain=plain)}")
        if gloss:
    ...
    def summarize_results(results: list[ExerciseResult], plain: bool) -> None:
        ...
        print(GuidedStoryPlayCli.style("Exercise summary", bold=True, plain=plain))
...
if __name__ == "__main__":
    raise SystemExit(GuidedStoryPlayCli.main())
