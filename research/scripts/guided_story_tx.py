...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_tx_start_parser(subparsers)
    add_tx_use_parser(subparsers)
...
def add_tx_start_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.set_defaults(handler=cmd_tx_start)
...
def add_tx_use_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.set_defaults(handler=cmd_tx_use)
...
def add_new_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    add_set_args(step)
    step.set_defaults(handler=cmd_new_step)

    screen = sub.add_parser("screen", help="Create a screen inside a step.")
    add_tx_selector_args(screen)
    ...
    add_set_args(term)
    term.set_defaults(handler=cmd_new_term)

    manifest_step = sub.add_parser("manifest-step", help="Create a manifest entry.")
    add_tx_selector_args(manifest_step)
    ...
    add_set_args(manifest_step)
    manifest_step.set_defaults(handler=cmd_new_manifest_step)
...
def add_edit_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    add_set_args(step)
    step.set_defaults(handler=cmd_edit_step)
    ...
    add_set_args(screen)
    screen.set_defaults(handler=cmd_edit_screen)
    ...
    add_set_args(term)
    term.set_defaults(handler=cmd_edit_term)
    ...
    add_set_args(manifest_step)
    manifest_step.set_defaults(handler=cmd_edit_manifest_step)
...
def add_delete_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    tx.set_defaults(handler=cmd_delete_tx)
    ...
    step.set_defaults(handler=cmd_delete_step)
    ...
    screen.set_defaults(handler=cmd_delete_screen)
    ...
    term.set_defaults(handler=cmd_delete_term)
    ...
    manifest_step.set_defaults(handler=cmd_delete_manifest_step)
...
def add_diff_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.set_defaults(handler=cmd_diff)
...
def add_save_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.set_defaults(handler=cmd_save)


def cmd_tx_start(args: argparse.Namespace) -> int:
    tx_name = validate_tx_name(args.name)
    source_rel = normalize_project_relpath(args.source_dir)
    ...
    copy_source_tree(source_abs, work_root / source_rel)
    copy_source_tree(source_abs, base_root / source_rel)
    write_json(
        tx_dir / "meta.json",
        {
            "name": tx_name,
...
def cmd_tx_use(args: argparse.Namespace) -> int:
    ...
    meta = read_json(tx_dir / "meta.json")
...
def cmd_new_step(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    manifest = load_manifest(tx)
    sequence_id = args.sequence_id
    existing_entry = find_manifest_entry(manifest, sequence_id)
    ...
    target_rel = (
        normalize_source_file_relpath(tx, args.file)
        if args.file
        else default_step_relpath(tx.source_rel, lesson_id, sequence_id)
    )
    if existing_entry:
        target_rel = normalize_source_file_relpath(tx, existing_entry["file"])
    ...
    apply_object_mutations(step_obj, args.set_json, args.set_str, args.unset)
    ensure_parent(target_path)
    write_json(target_path, step_obj)
...
def cmd_new_screen(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    step, step_path = load_step_by_sequence(tx, args.step_id)
    screens = step.setdefault("screens", [])
    if not isinstance(screens, list):
        raise GuidedStoryTxError(f"{args.step_id} has invalid screens data")

    screen_id = args.screen_id or next_screen_id(screens)
    ...
    apply_object_mutations(screen_obj, args.set_json, args.set_str, args.unset)
...
def cmd_new_term(args: argparse.Namespace) -> int:
    ...
    apply_object_mutations(term_obj, args.set_json, args.set_str, args.unset)
...
def cmd_new_manifest_step(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    manifest = load_manifest(tx)
    if find_manifest_entry(manifest, args.sequence_id):
        raise GuidedStoryTxError(f"manifest entry already exists: {args.sequence_id}")

    entry: dict[str, Any] = {
        "sequence_id": args.sequence_id,
        "file": normalize_source_file_relpath(tx, args.file).as_posix(),
        "concept": args.concept,
    }
    apply_object_mutations(entry, args.set_json, args.set_str, args.unset)
    normalize_manifest_entry_file(tx, entry)
    insert_at = choose_insert_index(
        items=manifest["steps"],
        before_id=args.before,
        after_id=args.after,
...
def cmd_edit_step(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    step, step_path = load_step_by_sequence(tx, args.sequence_id)
    manifest = load_manifest(tx)
    manifest_entry = find_manifest_entry(manifest, args.sequence_id)
    ...
    if args.plain_text:
        source = ensure_dict(step, "source")
        source["plain_text"] = args.plain_text
    if args.clear_related:
        ensure_dict(step, "source")["related"] = []
    if args.related:
        ensure_dict(step, "source")["related"] = list(args.related)

    apply_object_mutations(step, args.set_json, args.set_str, args.unset)
    ...
    if args.rename_sequence_id:
        ...
        other_manifest_entry = find_manifest_entry(manifest, new_sequence)
        ...
        other_step = find_step_path_by_sequence(tx, new_sequence)
        ...
        if manifest_entry:
            ...
            current_rel = normalize_source_file_relpath(tx, manifest_entry["file"])
            expected_old = default_step_relpath(tx.source_rel, lesson_id, old_sequence)
            if current_rel == expected_old or current_rel.name.endswith(f".{old_sequence}.json"):
                renamed_rel = default_step_relpath(tx.source_rel, lesson_id, new_sequence)
    ...
    if new_step_path != step_path and new_step_path.exists():
        raise GuidedStoryTxError(f"target step file already exists: {path_in_project(new_step_path)}")
    write_json(step_path, step)
    if new_step_path != step_path:
        ensure_parent(new_step_path)
...
def cmd_edit_screen(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    step, step_path = load_step_by_sequence(tx, args.step_id)
    screen, screen_index = find_screen(step, args.screen_id)
    ...
    apply_object_mutations(screen, args.set_json, args.set_str, args.unset)

    target_index = choose_insert_index(
        items=step["screens"],
        before_id=args.before,
        after_id=args.after,
        index=args.index,
...
def cmd_edit_term(args: argparse.Namespace) -> int:
    ...
    if args.rename_term_id:
        ...
        rename_term_references(step, args.term_id, args.rename_term_id)
...
def cmd_edit_manifest_step(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    manifest = load_manifest(tx)
    entry = find_manifest_entry(manifest, args.sequence_id)
    if not entry:
    ...
    if args.file:
        entry["file"] = normalize_source_file_relpath(tx, args.file).as_posix()
...
def cmd_delete_step(args: argparse.Namespace) -> int:
    ...
    if not args.keep_manifest_entry:
        remove_manifest_entry(manifest, args.sequence_id)
        save_manifest(tx, manifest)
...
def cmd_delete_screen(args: argparse.Namespace) -> int:
    tx = resolve_tx_from_args(args)
    step, step_path = load_step_by_sequence(tx, args.step_id)
    _, screen_index = find_screen(step, args.screen_id)
...
def cmd_delete_manifest_step(args: argparse.Namespace) -> int:
    ...
    step_path = tx.work_root / manifest_entry_file_relpath(tx, entry)
...
def cmd_diff(args: argparse.Namespace) -> int:
    ...
    changes = compute_directory_changes(left_dir, tx.work_source_dir)
    if not changes:
    ...
    for idx, rel_path in enumerate(changes):
        ...
        source_lines = read_text_lines(source_path)
        tx_lines = read_text_lines(tx_path)
...
def cmd_save(args: argparse.Namespace) -> int:
    ...
    diverged = compute_directory_changes(tx.base_source_dir, tx.source_abs)
    ...
    sync_directory(tx.work_source_dir, tx.source_abs)
    replace_directory(tx.work_source_dir, tx.base_source_dir)
    print(f"saved transaction {tx.name} into {path_in_project(tx.source_abs)}")
    return 0
...
def list_steps(tx: Transaction) -> int:
    manifest = load_manifest(tx)
    file_map = load_step_file_map(tx)
    for idx, entry in enumerate(manifest.get("steps", [])):
        rel_path = manifest_entry_file_relpath(tx, entry)
...
def list_screens(tx: Transaction, step_id: str) -> int:
    step, _ = load_step_by_sequence(tx, step_id)
...
def resolve_tx_from_args(args: argparse.Namespace) -> Transaction:
    tx_name = select_tx_name(args.tx, getattr(args, "current", False))
    return resolve_tx(tx_name)
...
def resolve_tx(name: str) -> Transaction:
    ...
    if not isinstance(meta_name, str) or validate_tx_name(meta_name) != tx_name:
    ...
    source_rel = normalize_project_relpath(source_rel_value)
    return Transaction(
        name=tx_name,
        root=tx_dir,
        work_root=tx_dir / "work",
...
def set_current_tx(name: str) -> None:
    safe_name = validate_tx_name(name)
    ensure_parent(TX_CURRENT_FILE)
...
def read_json(path: Path) -> Any:
    try:
        ...
        raise GuidedStoryTxError(f"missing JSON file: {path_in_project(path)}") from exc
    except json.JSONDecodeError as exc:
        raise GuidedStoryTxError(
            f"invalid JSON in {path_in_project(path)} at line {exc.lineno} column {exc.colno}: {exc.msg}"
        ) from exc
...
def replace_directory(source_abs: Path, dest_abs: Path) -> None:
    ...
    copy_source_tree(source_abs, dest_abs)
...
def save_manifest(tx: Transaction, manifest: dict[str, Any]) -> None:
    write_json(tx.work_source_dir / "manifest.json", manifest)
...
def remove_manifest_entry(manifest: dict[str, Any], sequence_id: str) -> None:
    entry = find_manifest_entry(manifest, sequence_id)
    if not entry:
...
def manifest_entry_file_relpath(tx: Transaction, entry: dict[str, Any]) -> Path:
    ...
    return normalize_source_file_relpath(tx, file_value)


def normalize_manifest_entry_file(tx: Transaction, entry: dict[str, Any]) -> None:
    entry["file"] = manifest_entry_file_relpath(tx, entry).as_posix()


def normalize_source_file_relpath(tx: Transaction, path_value: str | Path) -> Path:
    rel_path = normalize_project_relpath(path_value)
    try:
...
def find_step_path_by_sequence(tx: Transaction, sequence_id: str) -> Path | None:
    ...
    entry = find_manifest_entry(manifest, sequence_id)
    if entry:
        candidate = tx.work_root / manifest_entry_file_relpath(tx, entry)
        if candidate.exists():
    ...
    if not matches:
        return None
    if len(matches) > 1:
        rendered = ", ".join(path_in_project(path) for path in matches)
...
def parse_json_assignment(value: str) -> tuple[list[str], Any]:
    ...
    return parse_object_path(path_str), parsed
...
def parse_string_assignment(value: str) -> tuple[list[str], str]:
    ...
    if not path_str:
        raise GuidedStoryTxError(f"missing assignment path in: {value}")
    return parse_object_path(path_str), payload
...
def apply_object_mutations(
    ...
    for item in set_json_items:
        path, value = parse_json_assignment(item)
        set_path_value(obj, path, value)
    for item in set_str_items:
        path, value = parse_string_assignment(item)
        set_path_value(obj, path, value)
    for item in unset_items:
        remove_path_value(obj, parse_object_path(item))
...
def set_path_value(container: dict[str, Any], path: list[str], value: Any) -> None:
    ...
    for offset, key in enumerate(path[:-1]):
        ...
        if isinstance(current, list):
            idx = parse_index_token(key, len(current), allow_end=False)
    ...
    if isinstance(current, dict):
        ...
        return
    if isinstance(current, list):
        idx = parse_index_token(last, len(current), allow_end=False)
        current[idx] = value
        return
...
def remove_path_value(container: dict[str, Any], path: list[str]) -> None:
    ...
    if isinstance(current, dict):
        ...
        idx = parse_index_token(path[-1], len(current), allow_end=False)
        current.pop(idx)
    else:
...
def rename_term_references(step: dict[str, Any], old_id: str, new_id: str) -> None:
    for screen in step.get("screens", []):
        ...
        if isinstance(lines, list):
            screen["lines"] = [replace_term_id_tag(line, old_id, new_id) for line in lines]
        ...
        if isinstance(lines, list):
            ...
            for line in lines:
                if isinstance(line, str):
                    ...
                    line = replace_term_id_tag(line, term_id, term_id, drop_tag=True)
                cleaned.append(line)
...
def validate_transaction(tx: Transaction) -> ValidationResult:
    ...
    for idx, entry in enumerate(steps):
        ...
        try:
            step_rel = normalize_source_file_relpath(tx, file_value)
        except GuidedStoryTxError as exc:
...
def validate_step_file(
    *,
    ...
    rel_path = path_in_project(step_path)
    try:
        step = read_json(step_path)
    except GuidedStoryTxError as exc:
...
def sync_directory(source_dir: Path, dest_dir: Path) -> None:
    ...
    dest_files = collect_files(dest_dir)
