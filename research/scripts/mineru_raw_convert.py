...
def read_token() -> str:
    ...
    token_file = Path(env_str("MINERU_TOKEN_FILE", "mineru_token.txt"))
...
def request_upload_url(
    ...
    payload = {
        ...
        "language": env_str("MINERU_LANGUAGE", "ch"),
        "enable_table": env_bool("MINERU_ENABLE_TABLE", True),
        "model_version": env_str("MINERU_MODEL_VERSION", "vlm"),
        "files": [
            {
                "name": input_file.name,
                "is_ocr": env_bool("MINERU_IS_OCR", True),
    ...
    response = session.post(
        submit_url,
        json=payload,
        timeout=env_int("MINERU_REQUEST_TIMEOUT_SECONDS", 120),
...
def upload_file(upload_url: str, input_file: Path) -> None:
    with input_file.open("rb") as handle:
        response = requests.put(
            upload_url,
            data=handle,
            timeout=env_int("MINERU_UPLOAD_TIMEOUT_SECONDS", 600),
        )
    response.raise_for_status()
...
def poll_result(
    ...
    timeout_seconds = env_int("MINERU_RESULT_TIMEOUT_SECONDS", 3600)
    poll_interval = env_int("MINERU_POLL_INTERVAL_SECONDS", 15)
...
def download_zip(zip_url: str, archive_path: Path) -> None:
    with requests.get(
        zip_url,
        stream=True,
        timeout=env_int("MINERU_DOWNLOAD_TIMEOUT_SECONDS", 600),
    ) as response:
        response.raise_for_status()
...
def main() -> int:
    ...
    output_dir = resolve_output_dir(input_file, output_root, args.output_name).resolve()
    assert_paths(input_file, output_dir, args.force)

    token = read_token()
    base_url = env_str("MINERU_API_BASE_URL", "https://mineru.net")
    session = session_with_headers(token)

    print(f"[mineru] input={input_file}")
    print(f"[mineru] output_dir={output_dir}")
    print(f"[mineru] model_version={env_str('MINERU_MODEL_VERSION', 'vlm')}")

    batch_id, upload_url = request_upload_url(session, base_url, input_file)
    print(f"[mineru] batch_id={batch_id}")
    print("[mineru] upload=started")
    upload_file(upload_url, input_file)
    print("[mineru] upload=done")

    extract_result = poll_result(session, base_url, batch_id, input_file)
    ...
    download_zip(full_zip_url, archive_path)
    extract_archive(archive_path, extracted_dir)
    write_job_metadata(output_dir, batch_id, extract_result, input_file)

    markdown_path = pick_markdown(extracted_dir)
    ...
    if env_bool("MINERU_FAIL_ON_EMPTY_TEXT", False) and plain_size == 0:
