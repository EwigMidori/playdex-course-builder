...
HOST = "127.0.0.1"
PORT = int(os.environ.get("PROGRESS_VIEW_PORT", "18480"))


class ProgressPageRenderer:
                ...
                rows.append(
                    {
                        ...
                        "crate_scope": task.get("crate_scope", []),
    ...
    <body>
      <div class="page">
        <div class="hero">
          <div class="card hero-main">
            <h1>reso progress</h1>
            <div class="subtitle">A Notion-style execution view for <code>docs/progress.json</code></div>
            <div class="meta">
        ...
        <div class="toolbar">
          <div class="field">
            ...
            <input id="search" type="search" placeholder="Search title, task id, crate, dependency">
          </div>
          <div class="field">
            ...
            <table id="progress-table">
              <thead>
                <tr>
                  ...
                  <th style="width: 12%">Crate Scope</th>
                  <th style="width: 8%">Depends On</th>
                  <th style="width: 12%">Done When</th>
                </tr>
              </thead>
              <tbody id="rows"></tbody>
            </table>
        ...
        <div class="footer">Served by <code>scripts/view_progress.py</code> on port 9736.</div>
      </div>
      ...
      <script>
        ...
        function renderRows(rows) {
          els.rows.innerHTML = rows.map((row) => `
            <tr>
              ...
              <td>
                ${row.description ? `<div class="subtle">${escapeHtml(row.description)}</div>` : '<span class="subtle">-</span>'}
              </td>
              ...
              <td>${renderTags(row.crate_scope)}</td>
          ...
          document.getElementById("progress-table").hidden = !hasRows;
        }
        ...
        function renderSummary() {
          ...
          els.project.textContent = state.progress.project || "-";
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/":
            body = ProgressPageRenderer.build_page().encode("utf-8")
            self._send(HTTPStatus.OK, body, "text/html; charset=utf-8")
            return

        if parsed.path == "/api/progress":
            try:
                progress = ProgressPageRenderer.load_progress()
                progress["rows"] = ProgressPageRenderer.flatten_rows(progress)
...
class ProgressViewerServer:
    def main() -> None:
        server = ThreadingHTTPServer((HOST, PORT), Handler)
        print(f"Progress viewer: http://{HOST}:{PORT}")
        server.serve_forever()


if __name__ == "__main__":
    ProgressViewerServer.main()
